...
/**
 * 输出员工所有信息 且工资超过本部分的平均工资
 * 
 * @author root
 *
 */
public class QueryUserDeptInfoByGTAvgAge extends Configured implements Tool {

	private static String typeDept = "tag-dept:";
	private static String typeUser = "tag-user:";

	public static class QueryUserDeptInfoByGTAvgAgeMapper extends
	...
	public static class QueryUserDeptInfoByGTAvgAgeReduce extends
		...
		protected void reduce(Text inText, Iterable<TextArrayWritable> values,
			...
			for (TextArrayWritable arrayWritable : values) {
				...
				if (holeInfo[0].equals(typeDept)) {
					...
					// 代表是user的
					sum = sum + Long.parseLong(holeInfo[2]);
					num = num + 1;
					userList.add(new String[] { holeInfo[1], holeInfo[2],
							holeInfo[3] });
				}
			}
			//直接舍弃小数
			...
			for (int i = 0; i < deptList.size(); i++) {
				for (int j = 0; j < userList.size(); j++) {
					if (Long.parseLong(userList.get(j)[1]) < avg) {
						continue;
					}
					sb = new StringBuffer();
					sb.append(deptList.get(i)[0]).append(",")
							.append(userList.get(j)[0]).append(",")
							.append(userList.get(j)[1]).append(",")
							.append(userList.get(j)[2])
							.append("---"+sum+"---"+num)
							;
	...
	public int run(String[] arg0) throws Exception {
		...
		job.setJobName("QueryUserDeptInfoByGTAvgAge");

		job.setJarByClass(QueryUserDeptInfoByGTAvgAge.class);
		job.setMapperClass(QueryUserDeptInfoByGTAvgAgeMapper.class);
		job.setReducerClass(QueryUserDeptInfoByGTAvgAgeReduce.class);
	...
	public static void main(String[] args) throws Exception {
		...
		int ret = ToolRunner.run(MRUtil.getConfiguration(null),
				new QueryUserDeptInfoByGTAvgAge(), arg);
		System.exit(ret);
	}
