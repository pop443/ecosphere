...
public class CitySalarySum  extends Configured implements Tool {
	static class MyMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
				...
				for (Path path : paths) {
					...
					if (path.toString().contains("dept")) {
						...
						while (null != (deptIdName = in.readLine())) {
							// 对部门文件字段进行拆分并缓存到deptMap中
							// 其中Map中key为部门编号，value为城市名称
							map.put(deptIdName.split(",")[0], deptIdName.split(",")[2]);
						}
					}
				}
			} catch (IOException e) {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String city = map.get(kv[7]);
			//获取城市名称
			if (map.containsKey(kv[7])) {
				context.write(new Text(city), new IntWritable(Integer.parseInt(kv[5].trim())));
			}
		}
	}
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new CitySalarySum(), args);
		System.exit(res);
	}
}
